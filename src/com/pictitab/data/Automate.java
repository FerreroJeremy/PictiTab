...
public class Automate {
	...
	private ArrayList<Node> currentNodes; // current position in the eligible
											// rules
	 ...
	 *            (Grammar): Grammar.
	 * @param words
	 *            (ArrayList<Lexicon>): Lexicon entries.
	 * @param data
	 *            (AppData): Data
	 **/
	public Automate(Grammar gram, List<Lexicon> words, AppData data) {
		...
		for (int i = 0; i < gram.getRules().size(); i++) {
			Node tmp = this.createNodesFromRule(gram.getRules().get(i), 0,
					words);
			this.rules.add(tmp);
		}
		// At the beginning of the rules are eligible
	 ...
	 * 
	 * @param rule
	 *            (ArrayList<Category>): categories sequence representing the
	 *            rule of the grammar.
	 * @param numState
	 *            (int): Index of the depth of the current node in the rule.
	 * @param words
	 *            (List<Lexicon>): Lexicon entries.
	 * @return Current node.
	 **/
	private Node createNodesFromRule(ArrayList<Category> rule, int numState,
			List<Lexicon> words) {
		Node node = new Node();
		// rules of the grammar (the first element are always the category of
		// the current node)
		...
		node.setWords(this.getWordsFromCategory(node.getCategory(),
				remainingWords));
		node.setState(numState);
		if (endRule.size() > 0) {
			Node son = this.createNodesFromRule(endRule, numState + 1,
					remainingWords);
			son.setFather(node);
	 ...
	 **/
	public ArrayList<Lexicon> getWordsFromCategory(Category c,
			ArrayList<Lexicon> words) {
		Category c2 = data.getCategories().get(
				data.getCategoryByName(c.getName()));
		ArrayList<Lexicon> tmpWords = new ArrayList<Lexicon>();
		List<Category> subcategories = c2.getCategories();
		for (int i = 0; i < subcategories.size(); i++) {
			ArrayList<Lexicon> tmpLex = getWordsFromCategory(
					subcategories.get(i), words);
	...
	/**
	 * Go to the precedent state in the automate (flash back). Update the rules
	 * and the current node.
	 * 
	 * @param sentence
	 *            (ArrayList<Lexicon>): The remaining words of the sentence.
	 **/
	...
	/**
	 * Return all the words to draw/display to go to the next state of the
	 * automate.
	 * 
